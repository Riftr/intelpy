...
parser.add_argument('--ignore-pyqt', action='store_true',
                    help='Ignore pyqt in the requirements.txt file. Assume you are using the base package.')
...
for i in packages:
    if i["name"].lower().startswith("pyqt"):
        #if opts.ignore_pyqt:
        #    print("Ignoring pyqt, make sure to manually remove it from the output file")
        #    continue
        #else:
        print("PyQt packages are not supported by flapak-pip-generator. Just remove it from the requirements.txt.")
